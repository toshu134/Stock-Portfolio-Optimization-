...
def transformation( stocks , investment ):
    ...
    #converting into float just incase any string value
...
def MVO(expected_return_selected, cov_selected, total):
    ...
    o_weights = np.round(np.dot(rounded_weights,total),2)
    print("well")
    return {
            "optimal_investment": o_weights.tolist(),
...
def sharpe_ratio(expected_return2, cov2 , total ):
    ...
    o_wegihts = np.round(np.dot(max_sharpe_weights, total),2)
    
    return{
        "portfolio_return":max_sharpe_return,
        "portfolio_risk": max_sharpe_volatility,
        "portfolio_weights":o_wegihts
    }
